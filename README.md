...
├── plink.exe
├── README.md
├── ssh-tunnel-monitor.ahk      # главный скрипт. двойной клик на нем для запуска туннеля
...
Убедитесь, что файл `plink.exe` находится рядом со скриптами.

### 4. Запустите скрипт
Дважды щёлкните по файлу `ssh-tunnel-monitor.ahk`. При первом запуске откроется мастер настройки.
![состав проекта](./images/состав.jpg)
...
1. Запустить `ssh-tunnel-monitor.ahk`.
